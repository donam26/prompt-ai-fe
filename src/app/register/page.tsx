"use client";

import { useState, Suspense } from "react";
import { signIn } from "next-auth/react";
import { Button } from "@/components/ui/button";
import {
  Card,
  CardContent,
  CardDescription,
  CardHeader,
  CardTitle,
} from "@/components/ui/card";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Loader2, Mail } from "lucide-react";
import { AUTH_LABELS } from "@/constants";
import { ROUTES_URL } from "@/constants/routes-url";
import { useLoginQuery } from "@/hooks/auth/useLoginQuery";
import { useAuthRedirect } from "@/hooks/useAuthRedirect";
import { AuthLoading } from "@/components/ui/auth-loading";
import { trackLogin } from "@/lib/ga";
import Link from "next/link";

function RegisterContent() {
  const { isLoading: isLoginLoading, mutate: loginUser } = useLoginQuery();
  const { isLoading, isLoggedIn } = useAuthRedirect({
    redirectTo: ROUTES_URL.HOME,
    requireGuest: true,
  });
  const [email, setEmail] = useState("");
  const [isGoogleLoading, setIsGoogleLoading] = useState(false);

  const handleEmailRegister = (e: React.FormEvent) => {
    e.preventDefault();
    loginUser(email);
  };

  const handleGoogleRegister = async (): Promise<void> => {
    try {
      setIsGoogleLoading(true);

      // Track Google login attempt
      trackLogin("google");

      await signIn("google", {
        callbackUrl: ROUTES_URL.HOME,
        redirect: true,
      });
    } catch (error) {
      console.error("Google login error:", error);
      setIsGoogleLoading(false);
    }
  };

  // Show loading while checking authentication
  if (isLoading || isLoginLoading) {
    return <AuthLoading message="Đang kiểm tra trạng thái đăng nhập..." />;
  }

  // Don't render register form if user is already logged in
  if (isLoggedIn) {
    return null; // Will redirect via useAuthRedirect
  }

  return (
    <div className="flex justify-center items-center bg-white p-4 min-h-screen">
      <div className="w-full max-w-md">
        <div className="mb-8 text-center">
          <h1 className="font-bold text-gray-900 text-3xl">
            {AUTH_LABELS.REGISTER.TITLE}
          </h1>
          <p className="mt-2 text-gray-600">{AUTH_LABELS.REGISTER.SUBTITLE}</p>
        </div>

        <Card>
          <CardHeader className="space-y-0 px-6">
            <CardTitle>{AUTH_LABELS.REGISTER.FORM_TITLE}</CardTitle>
            <CardDescription>
              {AUTH_LABELS.REGISTER.FORM_DESCRIPTION}
            </CardDescription>
          </CardHeader>
          <CardContent className="px-6">
            <form onSubmit={handleEmailRegister} className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="email">Email</Label>
                <div className="relative">
                  <Mail className="top-3 left-3 absolute w-4 h-4 text-gray-400" />
                  <Input
                    id="email"
                    type="email"
                    placeholder={AUTH_LABELS.REGISTER.EMAIL_PLACEHOLDER}
                    value={email}
                    onChange={e => setEmail(e.target.value)}
                    className="pl-10"
                    required
                  />
                </div>
              </div>

              <Button
                type="submit"
                className="w-full"
                disabled={isLoginLoading}
              >
                {isLoginLoading && (
                  <Loader2 className="mr-2 w-4 h-4 animate-spin" />
                )}
                {AUTH_LABELS.REGISTER.SUBMIT}
              </Button>
            </form>

            {/* Divider */}
            <div className="relative my-8">
              <div className="absolute inset-0 flex items-center">
                <span className="border-gray-200 border-t w-full" />
              </div>
              <div className="relative flex justify-center text-xs uppercase">
                <span className="bg-white px-4 font-medium text-gray-500 tracking-wide">
                  Hoặc
                </span>
              </div>
            </div>

            {/* Google Register Button */}
            <Button
              type="button"
              variant="outline"
              className="bg-white hover:bg-gray-50 shadow-sm hover:shadow-md border-2 border-gray-200 hover:border-red-300 w-full h-12 font-medium text-gray-700 hover:text-red-600 transition-all duration-200"
              onClick={handleGoogleRegister}
              disabled={isGoogleLoading || isLoginLoading}
            >
              {isGoogleLoading ? (
                <Loader2 className="mr-3 w-5 h-5 text-red-500 animate-spin" />
              ) : (
                <svg className="mr-3 w-5 h-5" viewBox="0 0 24 24">
                  <path
                    fill="#4285F4"
                    d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                  />
                  <path
                    fill="#34A853"
                    d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                  />
                  <path
                    fill="#FBBC05"
                    d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                  />
                  <path
                    fill="#EA4335"
                    d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                  />
                </svg>
              )}
              <span className="font-semibold text-sm">Đăng ký với Google</span>
            </Button>

            {/* Login Link */}
            <div className="mt-6 text-center">
              <p className="text-gray-600 text-sm">
                {AUTH_LABELS.REGISTER.HAS_ACCOUNT}{" "}
                <Link
                  href={ROUTES_URL.LOGIN}
                  className="font-medium text-red-600 hover:text-red-700 hover:underline"
                >
                  {AUTH_LABELS.REGISTER.LOGIN_LINK}
                </Link>
              </p>
            </div>
          </CardContent>
        </Card>
      </div>
    </div>
  );
}

export default function RegisterPage() {
  return (
    <Suspense
      fallback={
        <div className="flex justify-center items-center min-h-screen">
          <div className="text-center">
            <Loader2 className="mx-auto mb-4 w-8 h-8 animate-spin" />
            <p>Loading...</p>
          </div>
        </div>
      }
    >
      <RegisterContent />
    </Suspense>
  );
}
